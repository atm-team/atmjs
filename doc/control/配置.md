...
```
{
  "stable": false,                        // 模块是否已经稳定
  "close": false,                         // 是否关闭,默认为false
  "debug": {
    "param": "debugId",                   // 调试参数
    "importCss": false                    // 联调时是否用@import的方式调用本地的css文件
  },
...
```
stable: 当前模块是否稳定,如过为false,则在模板中调用atmjs.use时会执行构建。建议在模块开发完毕后修改为true;
